...
class User extends Model
{
    ...
    public function getAll()
    {
        $obj_select = $this->connection
            ->prepare("SELECT * FROM users WHERE id != 0 ORDER BY updated_at DESC, created_at DESC");
        $obj_select->execute();
  ...
  VALUES(:username, :password)";
    ...
    public function getAllPagination($params = [])
    {
        ...
        $start = ($page - 1) * $limit;
        $obj_select = $this->connection
            ->prepare("SELECT * FROM users WHERE TRUE $this->str_search AND id !=0
...
VALUES(:username, :password, :first_name, :last_name, :phone, :address, :email, :avatar, :jobs, :facebook, :status)");
    ...
    public function update($id)
    {
        $obj_update = $this->connection
            ->prepare("UPDATE users SET password=:password, first_name=:first_name, last_name=:last_name, phone=:phone, 
